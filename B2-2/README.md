...
## (11. 중복저장 방지)
본 프로젝트에서는 핵심 기능(RSS 수집, AI 요약, 자동 저장, 예외 처리)을 우선 구현하였다. 동일 기사 중복 저장 방지는 URL 기반 검색을 통해 확장 가능하도록 설계하였으며, 현재는 권장 기능으로 남겨두었다.
중복저장방지는 TechCrunch RSS의 최신 기사 업로드 속도와 방대한 기사의 양을 고려하며 중복 기사 존재확룔이 낮다는 점을 두어 최우선이 아닌 이후 구현을 염두하여 제작하였다. 추가로,중복 저장 방지는 URL을 기준으로 구현할 예정이었으나,현재 사용한 Make의 Notion 모듈(Search Objects)은 데이터베이스 속성 검색을 지원하지 않아 기본 기능으로는 구현이 어려웠다. 실무에서는 Notion API Query Database 기능을사용하여 URL 중복 여부를 확인한 뒤 저장한다.
